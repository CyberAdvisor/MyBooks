...
Wiring in `app.js`: `state.listMode` gained `'series'` (alongside `'status'`/`'authors'`/`'categories'`/`'drilldown'`) and `state.drilldownType` gained `'series'` (alongside `'author'`/`'category'`). Two small lookup maps replaced what used to be inline ternaries once there were three drilldown types instead of two: `DRILLDOWN_LOOKUPS` (type -> the `logic.booksBy*` function to call) drives `renderDrilldownMode()`, and `DRILLDOWN_PARENT_MODE` (type -> which segmented-control mode the "‹ Back" button returns to) drives the drilldown back button. If you add a fourth browsable dimension, extend those two maps plus `renderListView()`'s dispatch and the `<button class="segment" data-mode="...">` row in `index.html` - that's the whole surface area.
...
`source` is a single string (one of `sourceList`, or `''`), not an array - a book has exactly one source. `sourceList` (`app.js`) is a dynamic, `localStorage`-persisted list (`SOURCE_STORAGE_KEY`) that works exactly like `categoryList`: grown via `addSourceIfNew()` from CSV import, the Edit screen's "+ Add new source..." sentinel (`buildSourceSelect()`/`handleSourceSelectChange()`, a copy-paste sibling of `buildCategorySelect()`/`handleCategorySelectChange()`), and reset-and-rebuilt on Restore from Backup. The one difference from `categoryList`: `sourceList` seeds itself with `DEFAULT_SOURCE_SEED` (`['Library', 'Kobo', 'Kindle', 'Yomu']`) the first time it's loaded on a browser with nothing in `localStorage` yet, rather than starting blank - `loadSourceList()` falls back to the seed (and immediately persists it) instead of `[]`. `db.emptyBook()` defaults a new book's `source` to `'Kindle'`.

This replaced two earlier versions in quick succession: first a multi-select chip UI using a native `prompt()` dialog to add a source, then an inline `.source-menu` popover to replace that dialog. The popover version had a real bug - it was appended to `document.body` with `position: fixed` computed from the "+ Add" chip's `getBoundingClientRect()`, which worked on desktop but was clipped off-screen on an iPhone's narrower viewport. Rather than chase viewport-clipping edge cases in a hand-rolled popover, source became single-select and reused the `<select>` pattern Category already had working reliably - simpler code, and one native form control iOS already knows how to position correctly. If a genuine need for multiple sources per book comes back, don't resurrect the popover; a native `<select multiple>` would be worth trying first.

Existing books that already had more than one source (from the old array-based `source: []`) keep only the first entry, via an explicit one-time migration - `migrateLegacyArraySource()`, called from `init()` before the first render. It scans every book and rewrites any `Array.isArray(book.source)` record to `book.source[0] || ''` via `db.updateBook()`. This is the same pattern flagged for a future `STATUS_ORDER` rename (see "Status list quick-action buttons" above) - actually written this time, since real data existed in this shape. It's cheap to leave running on every startup: once a book's `source` is a string, the `Array.isArray` check is false and it's skipped.

**The menu is appended to `<body>` with `position: fixed`, computed from the "+ Add" chip's `getBoundingClientRect()` - it is deliberately NOT nested inside the chip's own `.field-group`.** The first version got this wrong: it appended the menu as a plain absolutely-positioned child of `.source-add-wrap`, itself inside `.field-group`. `.field-group` has `overflow: hidden` (that's what gives it rounded corners), which silently clipped the popover to nothing - it was genuinely opening (the DOM node existed, the click handler fired correctly), just invisible, which read as "+ Add does nothing" rather than as a visibly broken popover. This is exactly the kind of bug `app.test.js` (jsdom, no real CSS layout/paint) cannot catch - the tests confirmed the menu *existed* in the DOM with the right items, not that it was actually visible on screen; if you add another popover anywhere in the app, check by hand (or note the risk explicitly) whether it lives inside any ancestor with `overflow: hidden`, since the test suite won't catch this class of bug on its own. `wrap._menu`/`wrap._closeMenuListener` (plain properties stashed on the `.source-add-wrap` element) track the detached-from-DOM-tree menu and its listener, since `wrap.querySelector('.source-menu')` no longer finds it once it's a child of `<body>` instead.

## Status list quick-action buttons

Each book row in the Status list view (not Authors/Categories/Series) gets an optional one-tap button, driven entirely by `logic.getStatusQuickAction(status)` (returns `{ label, nextStatus }` or `null`): Reading -> "Mark Read" -> `Read`; To Read -> "Read Now" -> `Reading`; Waiting -> "To Read" -> `To Read`; Read, Wanted, and Shelved get no button at all. `buildBookRow()` in `app.js` is the only caller - it used to take a `showMarkRead` boolean the caller computed (`status !== 'Read' && status !== 'Archive'`) and always wrote a hardcoded "Mark Read" -> `Read`; that's been replaced by asking `logic.getStatusQuickAction(book.status)` directly, so `buildBookRow()` no longer takes a second argument. If you add a new status to `STATUS_ORDER`, decide whether it needs an entry in `STATUS_QUICK_ACTIONS` too - it won't get a button by default (this is how `Wanted` was added - a new entry at the end of `STATUS_ORDER`, deliberately no `STATUS_QUICK_ACTIONS` entry, since there's no automatic "you've acquired it" signal to hang a button on). The CSS class is `.row-action-btn` (renamed from the old `.mark-read-btn`, since it no longer only means that) - the delegated click handler in `wireEvents()` checks for this class to avoid also triggering the row's own "open detail" click.

`Archive` was renamed to `Shelved` (same meaning - a book you own but don't intend to read, e.g. because it's no longer relevant - just a clearer name), and `Wanted` was added as a new status for books you want to add to your library but don't own yet (as distinct from `Waiting`, which is for a copy already being acquired - a hold placed, a pre-order, etc.). Current order: `Reading, To Read, Waiting, Read, Wanted, Shelved` - `Wanted` and `Shelved` are grouped at the end since they're both "not actively being read" states, with `Wanted` first since it's the earliest lifecycle stage (want it, don't have it) and `Shelved` last (have it, not reading it). Since `status` is stored as this literal string in IndexedDB, renaming a status value is a breaking change for any book already persisted with the old value - `Archive` had zero real-world users at the time of this rename (caught before shipping), so no migration was added; if you rename or remove a status again after books exist with the old value, you'll want one (a one-time pass in `init()` or similar that rewrites old values via `db.updateBook()`), since `groupBooksByStatus()` silently reassigns any status not in `STATUS_ORDER` to "To Read" rather than erroring.
...
- **`searchOpenLibrary()` throws on failure - a rejected `fetch` (network/CORS) or a non-ok HTTP status (e.g. Open Library returning 503 while overloaded) - instead of resolving to `[]`.** This used to swallow every failure to `[]` (just logging a `console.warn`), which meant a real outage looked pixel-identical in the UI to "this book genuinely isn't in Open Library's database" - a user has no way to tell "the service is down, try later" from "nothing here" when both say the same thing. Now only a genuine zero-result search (a successful request that just found nothing) resolves to `[]`; anything that failed to complete throws an `Error` with a short, already-user-presentable message (still `console.warn`s the fuller technical detail first, for debugging). Both call sites - `searchAddMatches` in Add Book and `refreshCurrentBookFromOnline` in Refresh - catch this and show it distinctly from the "No matches found" message (inline in the results area for Add Book, via `alert()` for Refresh, matching how each flow already surfaced the zero-result case). If you add a third caller of `findMatches`/`searchOpenLibrary`, it needs the same try/catch - don't let a thrown error surface as an unhandled rejection.
