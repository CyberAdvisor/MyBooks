...
## App versioning: bump on every deploy, always visible

`APP_VERSION` (`app.js`) is shown small and light next to the Library title (`#libraryVersion`), and doubles as a running changelog in the comment block right above it. Every script tag in `index.html` carries a matching `?v=<version>` cache-busting query string - GitHub Pages/Fastly serves the JS files with `cache-control: max-age=600`, so without this a browser (or the CDN) can keep serving stale JS for up to 10 minutes after a deploy with no way to force a fresh fetch short of a hard cache-clear. **On every change, bump `APP_VERSION` and every `?v=` in `index.html` together, in the same commit**, and add a one-line changelog entry. This isn't optional bookkeeping: it's the only way to confirm, by glancing at the app, that a given device is actually running what was just deployed rather than a stale cached copy - which mattered a lot working through the Dropbox sync issue below, where several rounds of "still broken" turned out to be stale JS, not the fix failing.

## Dropbox sync: tried and reverted (v1.1-v1.5)

Automatic Dropbox sync (OAuth 2.0 PKCE, App Folder scope, a single `library-backup.json` whole-library backup file) was built, shipped, and then removed after an unresolved, reproducible bug on a real device. This section exists so nobody re-attempts the same approach without knowing what was already ruled out.

**The symptom:** on one specific device, pulling from Dropbox reported success but loaded zero books, even though the file was independently confirmed correct and current (right size, right timestamp, right exact path - `Apps/<App Name>/library-backup.json`, verified via the Dropbox web UI, no duplicate app folders).

**What was ruled out, in order, each with real evidence, not guesses:**
1. **App-level sync logic bugs** - found and fixed several real ones along the way (a device with an empty local library could get silently wiped by a falsy-but-truthy empty-array pull; the manual push had no guard against overwriting a real backup with an empty one; sync direction relied on a timestamp marker instead of actual local data presence). These were genuine bugs and stayed fixed even after the eventual revert decision doesn't mean they didn't matter - it means fixing them wasn't sufficient.
2. **A missing OAuth scope** - real, and fixable (the Dropbox App Console's Permissions tab needed `files.content.write`/`files.content.read`/`files.metadata.read` explicitly checked, and any device connected before that fix needed to disconnect/reconnect to pick up a token with the new scopes).
3. **A content/ad blocker blocking the API-shaped download request** - plausible-sounding, so `downloadBackup()` was rewritten to go through `get_temporary_link` (on `api.dropboxapi.com`, already confirmed working) to get a plain, header-less `dl.dropboxusercontent.com` link instead of POSTing directly to `content.dropboxapi.com`. Made no difference.
4. **The browser's own HTTP cache serving a stale response to a reused temporary-link URL** - `fetch(link, { cache: 'no-store' })` was added. Made no difference: byte-identical stale content on repeat pulls.
5. **A network-level caching proxy** - a direct-POST fallback (POST requests are essentially never cached by intermediate proxies the way a GET to a file-shaped URL is) was added, tried automatically whenever the primary path came back suspiciously empty. Made no difference - the POST fallback got the exact same stale 2-byte `"[]"` content.
6. **A browser extension/content blocker intercepting requests below the fetch layer** - tested via Private/Incognito browsing. Same result (though this test is weaker than it looks on iOS Safari specifically, where Content Blocker extensions keep running in Private tabs by default - Private mode there doesn't actually disable them).

Points 4 and 5 together are the important ones: they rule out *every* caching layer that ordinary client-side JavaScript can control or route around (browser HTTP cache, browser cache API, and - by using a request type proxies don't cache - most realistic network-level caching too). Whatever was actually happening was consistent, reproducible, and immune to every standard workaround tried, on one specific real device, while working correctly during earlier testing on another. The likely remaining explanation is something at the browser-extension or OS/network-configuration level on that one device that no page-level code can detect or bypass - but this was never conclusively confirmed, because further live diagnosis stopped being worth the cost of continuing to debug across chat messages with no direct device/console access.

**The decision:** revert to manual-only sync (Backup Now / Restore from Backup File - both were already there and untouched by any of this). If Dropbox (or another provider) sync is attempted again, don't start from the assumption that any single fetch-layer trick will fix a "reports success but loads nothing" symptom - get real console/network access to the affected device before writing more workaround code, since every one of the five app-side attempts above ran into the same wall.
