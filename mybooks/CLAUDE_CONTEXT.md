...
There's no hardcoded list of categories. `categoryList` (`app.js`) starts empty and is persisted to `localStorage` (`CATEGORY_STORAGE_KEY`), growing three ways: CSV import folds in any non-blank `category` values it sees (`addCategoryIfNew()`, called from `handleCsvImport()`); Restore from Backup rebuilds it from scratch from the restored books (see below - this one resets rather than folds in); and the Edit screen's category `<select>` has a trailing "+ Add new category..." option (`ADD_NEW_CATEGORY_VALUE` sentinel, handled by `handleCategorySelectChange()`) that prompts for a name and adds it. `addCategoryIfNew()` dedupes case-insensitively and keeps the list alphabetically sorted; `getCategoryList()` is the read-only accessor other code (and the test suite) should use, since `categoryList` itself is a `let` binding and so isn't reachable as `window.categoryList`.
