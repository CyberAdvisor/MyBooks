...
- **Library**, grouped by Status (Reading / To Read / Waiting / Read / Wanted / Shelved) with collapsible sections and a search bar, or browsed by an Authors, Categories, or Series index (books with no category fall under "Uncategorized" rather than disappearing from that view; Series only lists books that actually have one set - there's no equivalent "no series" bucket). Wanted is for books you'd like to add to your library but don't own yet; Shelved is for books you own but don't intend to read (e.g. no longer relevant). Each row shows title, author, and - when set - series and where you have it (Kindle, Kobo, etc.); rows don't grow blank lines for fields that aren't set. Each book also gets a one-tap quick action to advance it: "Read Now" moves a To Read book into Reading, "To Read" moves a Waiting book into To Read, and "Mark Read" finishes a book you're Reading.
...
- **Full detail/edit screen** - title, author, series and series number, category, status, star rating (1-5, color-coded), where you have it (a single-select dropdown - Library, Kobo, Kindle, and Yomu to start, plus "+ Add new source..." for anything else, same pattern as Category), synopsis, and free-text notes
...
- **Import** books in bulk from a CSV file (a ready-to-use example is included: `sample-import.csv`); duplicate title+author pairs are skipped automatically. The CSV format doesn't carry cover images, so imported books start without one - see "Filling in covers after a CSV import" below for how to add them, one at a time or in bulk. Neither the category list nor the source list is fixed - both grow from whatever values your CSV rows carry, plus anything you add later via "+ Add new category..."/"+ Add new source..." on the Edit screen.
